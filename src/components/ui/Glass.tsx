export function Glass({ className = "", children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={`bg-white/[0.04] backdrop-blur-2xl ${className}`} {...props}>{children}</div>;
}

export function GlassPill({ className = "", children, ...props }: React.HTMLAttributes<HTMLSpanElement>) {
  return <span className={`inline-flex items-center rounded-full bg-white/[0.04] px-2.5 py-0.5 text-[11px] font-medium text-white/60 backdrop-blur-2xl ${className}`} {...props}>{children}</span>;
}
...
export function GlassButton({ className = "", children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & React.AnchorHTMLAttributes<HTMLAnchorElement>) {
  ...
  return <Tag className={`inline-flex items-center gap-1.5 bg-white/[0.04] px-3 py-1.5 text-sm text-white/50 backdrop-blur-2xl transition-all hover:bg-white/[0.08] hover:text-white active:scale-[0.97] ${className}`} {...(props as any)}>{children}</Tag>;
}

export function GlassCard({ title, description, className = "" }: { title: string; description: string; className?: string }) {
  return (
    <Glass className={`group p-5 transition-all duration-300 hover:scale-[1.02] hover:bg-white/[0.06] ${className}`}>
  ...
  return (
    <div className="overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="bg-white/[0.01] transition-colors hover:bg-white/[0.04]">
